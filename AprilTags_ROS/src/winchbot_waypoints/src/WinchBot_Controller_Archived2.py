...
def pickup():
	...
	hook_centre = np.array((qv_mult((pos_rot.w,pos_rot.x,pos_rot.y,pos_rot.z),(0,-1.2,0))))
	#print(hook_centre)
	print([pos.x*39.3701,pos.y*39.3701,pos.z*39.3701])
	print([pos.x*39.3701,pos.y*39.3701,pos.z*39.3701] - cam_to_lead)
	print([pos.x*39.3701,pos.y*39.3701,pos.z*39.3701] - cam_to_lead + hook_centre)
	print(hook_centre)
	goal_pos = np.array([pos.x*39.3701,pos.y*39.3701,pos.z*39.3701]) + hook_centre - cam_to_lead
	print('Goal:')
	print(goal_pos)
	...
	platform_centre = np.array((qv_mult((effector_rot.w,effector_rot.x,effector_rot.y,effector_rot.z),(0,0,2))))*-1
	...
	while goal_reached==0:
		...
		if stage == 1:
			...
			stage+=1
		if stage == 2:
			target = [goal_pos[0]+x_offset,goal_pos[1]+y_offset2,goal_pos[2]+z_offset]
			q = winchbot_IK(winches, target)
			print('target=')
			print(target)
			q = np.array(q)
