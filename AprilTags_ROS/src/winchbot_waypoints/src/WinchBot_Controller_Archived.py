...
def pickup():
	...
	hook_centre = np.array((qv_mult((pos_rot.w,pos_rot.x,pos_rot.y,pos_rot.z),(0,1.2,0))))*-1
	#print(hook_centre)
...
# Find a connected ODrive (this will block until you connect one)
	...
	# CALIBRATION INACCURATE!!!
	calibration = np.array([end_effector.x*39.3701,end_effector.y*39.3701,end_effector.z*39.3701]) # Calibration AprilTag pos relative to Camera
	platform_centre = np.array((qv_mult((effector_rot.w,effector_rot.x,effector_rot.y,effector_rot.z),(0,0,2))))
	platform_cam = calibration + platform_centre # Platform centre pos relative to Camera
	print('Platform Centre:')
	print(platform_centre)
	platform = platform_cam - cam_to_lead # Platform centre pos relative to Lead Screw
	...
	drum_diam = 4.125 # Drum diameter in inches
	safe_z = 35 # safe Z coordinate for translation over the object
	#goal_pos[1]=goal_pos[1]+2.5 # Adjust for hook vs apriltag location on target
	x_offset = 0
	y_offset = 4.2
	z_offset = -4
	'''while goal_reached==0:
